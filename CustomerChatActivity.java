        ...
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (!snapshot.hasChild(user)){
                    ...
                    finish();
                    startActivity(new Intent(getApplicationContext(),DriversHomeActivity.class));
                }
                for (DataSnapshot chat : snapshot.child(user).getChildren()){
                    ...
                    databaseReference1.addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            arrayListTechnicians.add(snapshot.child("firstNamee").getValue().toString()+" "+snapshot.child("secondNamee").getValue().toString());
                            arrayListMobile.add(Objects.requireNonNull(snapshot.child("mobilee").getValue()).toString());
